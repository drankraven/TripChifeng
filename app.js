const routeDays = [
  {
    ...
    hotel: "住经棚镇核心区，优先带停车场、近镇内补能点的酒店；想泡温泉可住热水塘镇（距经棚约 20 km）。",
    charging: "林东满电出发；经棚镇是克什克腾主要补能点，今晚过夜补到 90% 左右。",
    summary:
      "周一多数博物馆闭馆，今天以户外与寺庙为主：辽上京国家考古遗址公园（露天遗址，周一可入）+ 召庙·真寂之寺石窟；下午北上巴林右旗看荟福寺、格斯尔庙，再翻越到大兴安岭余脉下的经棚镇。",
    stops: ["辽上京遗址(露天)", "召庙·真寂之寺", "大板·荟福寺/格斯尔庙", "经棚镇"]
  },
  {
    day: 2,
    date: "7月7日 周二",
    title: "克什克腾·达里诺尔湖 + 阿斯哈图石林",
    legs: [
      { from: "经棚镇", to: "达里诺尔湖", km: "约 80 km", time: "1.5 h（达达线北段）" },
      { from: "达里诺尔湖", to: "阿斯哈图石林", km: "约 135 km", time: "2.5 h（达达线）" },
      { from: "阿斯哈图石林", to: "经棚/热水塘", km: "约 90 km", time: "1.5 h" }
    ],
    totalDrive: "全天自驾约 305 km / 5.5 h（不含游玩）",
    stay: "住经棚镇或热水塘温泉镇",
    meals: "午餐 达里诺尔湖边：全鱼宴（华子鱼、鲫鱼）；晚餐 经棚/热水塘：温泉酒店或蒙餐。",
    hotel: "经棚镇吃饭补能方便；热水塘镇可泡温泉解乏（距经棚约 20 km，镇上有温泉酒店）。",
    charging:
      "经棚满电出发；达里诺尔/阿斯哈图一带补能点极少，务必在经棚补满；返程到经棚或热水塘再过夜补能。",
    summary:
      "走著名的“达达线”串联两大地质奇观：上午达里诺尔湖（火山堰塞湖、观鸟、湖岸），下午阿斯哈图花岗岩石林（世界地质公园）。今天长距离但风景连续，补能要前置。",
    stops: ["达里诺尔湖", "阿斯哈图石林", "热水塘温泉(可选)"]
  },
  {
    day: 3,
    date: "7月8日 周三",
    title: "经棚 → 乌兰布统草原",
    legs: [
      { from: "经棚/热水塘", to: "乌兰布统草原", km: "约 130 km", time: "2.5-3 h（经御道口）" }
    ],
    totalDrive: "全天自驾约 130 km / 2.5-3 h（不含游玩）",
    stay: "住乌兰布统景区内或围场县",
    meals: "午餐 乌兰布统：草原农家、手把肉、奶茶；晚餐 景区内或围场：烧烤、铁锅炖。",
    hotel: "想看日落星空住景区内（木屋/蒙古包）；想稳妥吃饭补能住围场县（距景区约 30-40 km）。",
    charging: "经棚满电出发；乌兰布统景区内补能点少且可能排队，到围场县补能更稳，今晚补到 85%-90%。",
    summary:
      "今天轻松些：一早从经棚南下到乌兰布统，全天交给草原——影视基地、欧式风光区、红山军马场、蛤蟆坝，傍晚在草原上看日落。",
    stops: ["乌兰布统草原", "影视基地/欧式风光区", "红山军马场/蛤蟆坝"]
  },
  {
    day: 4,
    date: "7月9日 周四",
    ...
    totalDrive: "全天自驾约 450-500 km / 6.5-7.5 h（不含午餐休整）",
    stay: "终点沈阳，不返赤峰",
    meals: "早午餐在乌兰布统或围场；途中在承德/平泉一带午餐休整；傍晚到沈阳吃老边饺子、烧烤或东北菜。",
    hotel: "当晚住沈阳；若次日继续行程，优先住靠近高速口或火车站一带。",
    charging: "乌兰布统满电出发；围场/承德或平泉补能一次（300 km 纯电必补）；进沈阳前补到 70% 以上。",
    summary:
      "返程日。建议早出发，走围场/承德方向回沈阳（比绕经赤峰短）。中途午餐+补能一次，傍晚抵达沈阳，结束赤峰行程。若想顺路看城市文化，可在沈阳前经赤峰市区，但会增加约 2 小时路程。",
    stops: ["乌兰布统", "围场/承德补给", "沈阳市区"]
  }
];
...
const chargePlan = [
  ...
  {
    day: "D2",
    legs: "经棚 → 达里诺尔 → 阿斯哈图 → 经棚",
    rule: "达里诺尔/阿斯哈图一带几乎无桩，务必在经棚补满再出发，返程回经棚再补。"
  },
  {
    day: "D3",
    legs: "经棚 → 乌兰布统",
    rule: "经棚满电出发；乌兰布统景区内桩少，到围场县补能更稳。"
  },
  {
    day: "D4",
    legs: "乌兰布统 → 围场/承德 → 沈阳",
...
const points = [
  ...
  { name: "克什克腾·经棚镇", day: 1, lat: 43.253, lng: 117.5, kind: "city" },
